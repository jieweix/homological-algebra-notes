...
\begin{defn}
Let $A_\bullet$ be a chain complex. The $n$th \textit{homology group} is $H_n(A_\bullet)=\ker(d:A_n\rightarrow A_{n-1})/\im(d:A_{n+1}\rightarrow A_n)$. Similarly define cohomology group of cochain complexes.
\end{defn}
...
\item If $M$ is a trivial $\Z[G]$-module then 1-cycles are indeed precisely group homomorphisms, and hence $H^1(G,M)=\frac{\Hom_\Grp(G,M)}{0}=\Hom_\Grp(G,M)$. For example, if $G$ is finite then $H^1(G,\Z)=0$.
\end{itemize}
\end{remark}
